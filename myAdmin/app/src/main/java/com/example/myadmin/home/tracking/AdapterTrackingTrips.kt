...
import com.example.myadmin.databinding.TrackItemTripsBinding

class AdapterTrackingTrips(private val list: List<com.example.myadmin.home.showtrips.availabletrips.Data?>, private val listener: TrackingTripListener): RecyclerView.Adapter<AdapterTrackingTrips.TripsHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TripsHolder {
        val view=LayoutInflater.from(parent.context).inflate(R.layout.track_item_trips,parent,false)
    ...
    override fun onBindViewHolder(holder: TripsHolder, position: Int) {
        ...
        holder.binding.trackingBtn.setOnClickListener {
            listener.onTrackingTrip(list[position]!!)
        }
    }
    class TripsHolder(item: View):RecyclerView.ViewHolder(item) {
        val binding= TrackItemTripsBinding.bind(item)
    }
}