package com.example.myadmin.home.showtrips.bookable
...
import com.example.myadmin.databinding.FragmentBookableTripsBinding
...
import com.example.myadmin.home.showtrips.availabletrips.AvailableTripListener
...
class BookableTrips : Fragment() , CityClickLestener, AvailableTripListener {
    lateinit var binding:FragmentBookableTripsBinding
    ...
    override fun onCreateView(
        ...
        binding= FragmentBookableTripsBinding.inflate(inflater,container,false)
        viewModel.getCities()
        kotlin.runCatching {
        ...
        binding.reset.setOnClickListener {
            ...
            viewModel.allTrips.observe(viewLifecycleOwner, Observer { result->
                kotlin.runCatching {
                    if(result!=null && result.success==true){
                        result.data!!.forEach { item->
                            if (item!!.numberdisksisFalse!!.size!=0){
                                trips.add(item)
                            }
                        }
                        //trips= result.data as MutableList<Data?>
