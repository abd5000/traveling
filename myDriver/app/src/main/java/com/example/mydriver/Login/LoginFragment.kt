...
import com.example.mydriver.databinding.FragmentLoginBinding
...
class LoginFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.login.setOnClickListener { v->
            ...
            if (checkTheEditText(email,password)) {
                ...
                kotlin.runCatching {
                    viewModel.diver.observe(requireActivity(), Observer{ user ->
                        if (user!=null) {
                            if (user.success == true) {
                                ...
                                Log.i("diverid", "${user.data.result!!.id}")
                                saveDriverId(user.data.result?.id!!)
    ...
    private fun saveDriverId(id:Int){
